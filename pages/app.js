...
import { Calendar, Truck, Package, MapPin, Phone, Mail, CheckCircle, Loader } from 'lucide-react';

// Supabase configuration
const SUPABASE_URL = 'https://jfegvkzglxwlmbdmekpz.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImpmZWd2a3pnbHh3bG1iZG1la3B6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjMxNTQwNzUsImV4cCI6MjA3ODczMDA3NX0.6A8T1fTtrmQspl8RhZ00DwUxPpL_5Rcq23QQvjXVE8s';
...
export default function SkipHireApp() {
  ...
  const loadPostcodePricing = async (postcode) => {
    ...
    try {
      const pricing = await supabaseFetch('postcode_pricing', { select: '*' });
  ...
  const handleAddNewCustomer = async () => {
    if (!newCustomer.firstName || !newCustomer.lastName || !newCustomer.email || 
        !newCustomer.phone || !newCustomer.address || !newCustomer.postcode) {
  ...
  const handleProceedToPayment = () => {
    ...
    if (bookingFormType === 'custom' && (!formData.customSkipSize || !formData.customPrice)) {
  ...
  const handleCreateBooking = async () => {
    ...
    try {
      const bookingData = {
        ...
        skip_size: bookingFormType === 'custom' ? formData.customSkipSize : formData.skipSize,
        booking_type: bookingFormType === 'custom' ? 'Custom' : formData.bookingType,
        ...
        payment_status: paymentMethod === 'COD' || paymentMethod === 'Call Back to Pay' ? 'Awaiting Payment' : 'Paid',
  ...
  const getCollectionRequests = () => {
    return bookings.filter(b => b.collectionStatus === 'Collection Requested');
  };
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      <div className="w-64 bg-gray-900 text-white p-4">
        ...
        <nav className="space-y-2">
          ...
          <button
            ...
            className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg ${
              currentView === 'collections' ? 'bg-blue-600' : 'hover:bg-gray-800'
            }`}
          >
            <Truck size={20} />
            ...
            <div className="space-y-4">
              {bookings.length === 0 ? (
                <div className="bg-white rounded-lg shadow p-8 text-center text-gray-500">
                  No bookings yet. Click "New Standard Booking" or "New Custom Booking" to create one.
                </div>
              ) : (
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center space-x-4 mb-2">
                          ...
                          <span className={`px-3 py-1 rounded text-sm ${
                            booking.collectionStatus === 'Collected' ? 'bg-green-100 text-green-800' : 
                            booking.collectionStatus === 'Collection Requested' ? 'bg-orange-100 text-orange-800' :
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
                          ...
                          <div className="flex items-center space-x-2">
                            <Calendar size={16} />
                            <span>Delivery: {booking.deliveryDate} ({booking.deliverySlot})</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <Package size={16} />
                            <span>{booking.skipSize} Skip - {booking.bookingType}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <Truck size={16} />
                        ...
                        <div className="mt-3 text-sm">
                          <span className="font-semibold">Total: £{booking.totalIncVAT.toFixed(2)}</span>
                          <span className="ml-2 text-gray-600">(Ex VAT: £{booking.totalExVAT.toFixed(2)})</span>
                          <span className="ml-4 text-blue-600">Payment: {booking.paymentMethod}</span>
                          {booking.cardReceiptNumber && (
                            <span className="ml-2 text-gray-600">(Receipt: {booking.cardReceiptNumber})</span>
                          )}
                          {booking.placement === 'Council Permit' && (
                            <span className="ml-4 text-orange-600">(Includes £{booking.permitPrice.toFixed(2)} permit - No VAT)</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col space-y-2">
                        ...
                        {booking.deliveryStatus === 'Delivered' && booking.collectionStatus === 'Awaiting Collection Request' && (
                          <button
                        ...
                        {booking.collectionStatus === 'Collection Requested' && (
                          <button
                  ...
                  <div className="space-y-4">
                    {getTodaysDeliveries().map(booking => (
                      <div key={booking.id} className="border-l-4 border-blue-500 bg-gray-50 p-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <div className="font-semibold">{booking.customerFirstName} {booking.customerLastName}</div>
                            <div className="text-sm text-gray-600">{booking.address}, {booking.postcode}</div>
                            <div className="text-sm text-gray-600">{booking.skipSize} - {booking.deliverySlot}</div>
                          </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="space-y-1 text-sm text-gray-600">
                          <div>{booking.address}, {booking.postcode}</div>
      ...
      {/* Rest of the modals will continue in next message... */}
    </div>
  );
{/* Booking Form Modal */}
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full my-8">
            <div className="p-6 max-h-[85vh] overflow-y-auto">
              <h2 className="text-2xl font-bold mb-6">
                {bookingFormType === 'standard' ? 'New Standard Booking' : 'New Custom Booking'}
              </h2>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {customerSearch && !selectedCustomer && getFilteredCustomers().length > 0 && (
                    <div className="mt-2 border rounded shadow-lg bg-white">
                      {getFilteredCustomers().map(customer => (
                        ...
                        >
                          ...
                          <div className="text-sm text-gray-600">{customer.address}, {customer.postcode}</div>
                        </button>
                      ))}
                    </div>
                  )}
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <div className="font-semibold">
                            ...
                            {selectedCustomer.company && <span className="text-gray-600"> ({selectedCustomer.company})</span>}
                          </div>
                          <div className="text-sm text-gray-600">{selectedCustomer.address}, {selectedCustomer.postcode}</div>
                          <div className="text-sm text-gray-600">{selectedCustomer.email} • {selectedCustomer.phone}</div>
                        </div>
                        <button
                          onClick={() => {
                            setSelectedCustomer(null);
                ...
                {bookingFormType === 'standard' && selectedCustomer && (
                  <>
                    {availableSkips.length > 0 ? (
                      <div>
                        <label className="block text-sm font-medium mb-1">Skip Size *</label>
                        <select
                        ...
                        >
                          ...
                          {availableSkips.map(skip => (
                            <option key={skip.size} value={skip.size}>
                              {skip.size} - £{skip.price} + VAT (£{(skip.price * 1.2).toFixed(2)} inc VAT)
                            </option>
                          ))}
                        </select>
                      </div>
                      ...
                      <div className="bg-yellow-50 border border-yellow-200 rounded p-3 text-sm text-yellow-800">
                        No skip pricing found for postcode {selectedCustomer.postcode}. Please use Custom Booking instead.
                      </div>
                    )}
                ...
                {bookingFormType === 'custom' && selectedCustomer && (
                  <>
                    <div>
                      <label className="block text-sm font-medium mb-1">Skip Size / Description *</label>
                      <input
                        type="text"
                        value={formData.customSkipSize}
                        onChange={(e) => setFormData({...formData, customSkipSize: e.target.value})}
                      ...
                      {bookingFormType === 'standard' && (
                        <>
                          <div className="flex justify-between mb-2">
                            ...
                            <span className="font-semibold">£{selectedSkipPrice.toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between mb-2 text-sm text-gray-600">
                            <span>VAT @ 20%:</span>
                            <span>£{(selectedSkipPrice * VAT_RATE).toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between mb-2">
                            <span>Skip Hire (Inc VAT):</span>
                            <span className="font-semibold">£{(selectedSkipPrice + (selectedSkipPrice * VAT_RATE)).toFixed(2)}</span>
                          </div>
                          {formData.placement === 'Council Permit' && (
                            <>
                              <div className="flex justify-between mb-2 text-orange-600 border-t pt-2">
                                <span>Permit Fee (No VAT):</span>
                                <span className="font-semibold">£{permitPrice.toFixed(2)}</span>
                              </div>
                            </>
                          )}
                        </>
                      ...
                      {bookingFormType === 'custom' && (
                        <>
                          <div className="flex justify-between mb-2">
                            ...
                            <span className="font-semibold">£{(parseFloat(formData.customPrice) || 0).toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between mb-2 text-sm text-gray-600">
                            <span>VAT @ 20%:</span>
                            <span>£{((parseFloat(formData.customPrice) || 0) * VAT_RATE).toFixed(2)}</span>
                          </div>
                        </>
                      )}
                      <div className="flex justify-between text-lg font-bold border-t pt-2 mt-2">
                        <span>Total to Pay:</span>
                        <span>£{calculatePricing().totalIncVat.toFixed(2)}</span>
                      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-lg shadow-xl max-w-xl w-full my-8">
            <div className="p-6 max-h-[85vh] overflow-y-auto">
              ...
              <p className="text-gray-600 mb-6">How will the customer pay for this booking?</p>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded p-4 mb-6">
                ...
                <div className="text-sm text-gray-700">
                  {bookingFormType === 'custom' ? formData.customSkipSize : formData.skipSize} Skip - 
              ...
              <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center p-3 border rounded cursor-pointer hover:bg-gray-50">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-600">Customer paid by card during call</div>
                      </div>
                    </label>
                    ...
                    <label className="flex items-center p-3 border rounded cursor-pointer hover:bg-gray-50">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-600">Driver will collect payment on delivery</div>
                      </div>
                    </label>
                    ...
                    <label className="flex items-center p-3 border rounded cursor-pointer hover:bg-gray-50">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-600">Send invoice for payment within terms</div>
                      </div>
                    </label>
                    ...
                    <label className="flex items-center p-3 border rounded cursor-pointer hover:bg-gray-50">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-600">Email customer a Stripe payment link</div>
                      </div>
                    </label>
                    ...
                    <label className="flex items-center p-3 border rounded cursor-pointer hover:bg-gray-50">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-600">Customer will call back later to pay by card</div>
                      </div>
                    </label>
                  </div>
                </div>
                ...
                {paymentMethod === 'Card Over Phone' && (
                  <div>
                    <label className="block text-sm font-medium mb-1">Card Receipt Number *</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Payment Notes (Optional)</label>
                  <textarea
                    value={paymentNotes}
